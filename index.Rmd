...
Table \@ref(fig:dataview) allows exploration of a few variables of the passwords dataset; the password, its popularity rank, associated password category and its strength on a scale of 1-10. 
...
ranking <- strong %>% filter(strength<=10) %>%
  arrange((rank)) %>%
  head(10) %>%
  ggplot(aes(y = password, x = offline_crack_sec)) +
  geom_point() +
