...
```{r length, fig.cap = "Most popular password lengths", eval=FALSE}
# This is the original code chunk for Figure 3.4
...
```{r figure-improvement-one}
# This is the improvement for Figure3.4

# Create a new variable measuring the length of each password in the list
length_passwords <- clean_passwords %>%
  mutate(length = nchar(password)) 

# Create a density and histogram to show the passwords length distribution
plot_density_histogram <- length_passwords %>% 
  #arrange(length) %>% 
  ...
  geom_histogram(binwidth = 1,
                 ...
                 fill = "blue",
                 alpha = 0.3) +
  geom_density(aes(y = 0.001*..count..),
               colour = "blue",
               fill = "blue", 
               alpha = 0.3) + 
  ...
  ggtitle("Password lengths distribution")
plot_density_histogram


# Create a bar chart to show the number of each passwords length by each category.
plot_bar <- length_passwords %>%
  mutate(category = fct_rev(fct_infreq(category))) %>%
  ggplot(aes(x = length)) +
  geom_bar() +
  theme_bw() +
...
grid.arrange(plot_density_histogram,
             plot_bar,
             ncol = 1)
...
It's interesting how linear the realtionship among the online and offline crack times across all password categories. The crack times (both online and offline) of passwords belonging to the simple-alphanumeic category are spread across a wide range of values while the passwords belonging to the rebellious-rude category are only spread across a smaller range of values.

Figure \@ref(fig:error-bar) represents the top 10 passwords with the highest difference of online and offline crack time. The red dots represent the offline password crack time in secods while the black dots represent the online password crack time in seconds. It's interesting how all top 10 passwords with the highest differences record a lower offline crack time in comparison to the online crack time.
