...
> What are the characteristics of the most common passwords? 
...
datatable(view_passwords, rownames = FALSE, caption = "Top 500 Most Popular Passwords", class = "cell-border stripe")
```
...
wordcloud(words = ranked_passwords$password,
          freq = ranked_passwords$freq,
          scale = c(3,.2),
          colors = brewer.pal(8, "Accent")[factor(ranked_passwords$category)])
```
...
plot_category <- category_passwords %>%
  ...
  ggtitle("Most popular password categories")
...
plot_length <- length_passwords %>%
  ...
  ggtitle("Most common password lengths")

interactive(plot_length)
```
...
summary_combined <- bind_rows(summary_all, 
  ...
  replace_na(list(category = "Overall"))
